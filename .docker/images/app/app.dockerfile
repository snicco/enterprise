...
#
COPY ./bin/third-party/php-scoper-0.17.5.phar /usr/local/bin/php-scoper
COPY ./bin/third-party/composer-normalize-2.28.3.phar /usr/local/bin/composer-normalize
RUN chmod +x /usr/local/bin/composer-normalize && chmod +x /usr/local/bin/php-scoper
...
ENV MONOREPO_PATH=$MONOREPO_PATH
