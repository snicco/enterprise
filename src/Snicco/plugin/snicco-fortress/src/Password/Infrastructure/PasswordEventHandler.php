...
use Snicco\Enterprise\Fortress\Password\Domain\Exception\InsufficientPasswordEntropy;
use Snicco\Enterprise\Fortress\Password\Domain\Exception\InsufficientPasswordLength;
use Snicco\Enterprise\Fortress\Password\Domain\Exception\PasswordLengthExceeded;
use Snicco\Enterprise\Fortress\Password\Domain\PasswordPolicy;
use Snicco\Enterprise\Fortress\Password\Infrastructure\MappedEvent\ResettingPassword;
use Snicco\Enterprise\Fortress\Password\Infrastructure\MappedEvent\UpdatingUserInAdminArea;
use WP_User;
...
final class PasswordEventHandler implements EventSubscriber
{
     ...
     */
    public function __construct(PasswordPolicy $policy, array $excluded_roles = [])
    {
        $this->policy = $policy;
    ...
    public function onPasswordReset(ResettingPassword $event): void
    {
        ...
        try {
            $this->policy->check($event->password(), $context);
        } catch (InsufficientPasswordLength $e) {
            $event->addError(
                'insufficient_pw_length',
                __('Passwords must have at least 12 characters.', 'snicco-auth')
