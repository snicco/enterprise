...
/**
 * Composer handles autoloaded files by generating a hash based on the
 * package-name that includes the file.
 *
 * Each unique hash will only be included once by the composer autoloader. This
 * means that if one of our plugins uses a package with autoloaded files there
 * is the same possibility of race conditions if that package is used in user
 * land elsewhere.
 *
 * What we do here is changing the generated hash for our dependencies in our
 * autoloder. We need to do that in two files: 1)
 * vendor/composer/autoload_files.php 1) vendor/composer/autoload_static.php
 *
